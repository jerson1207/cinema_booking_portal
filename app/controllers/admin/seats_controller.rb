class Admin::SeatsController < ApplicationController
  ...
  def index
    ...
    @seats = Cinema.find(params[:cinema_id]).seats.all
  end

  # GET /seats/1 or /seats/1.json
  def show
    @cinema = Cinema.find(params[:cinema_id])
    @seat = Cinema.find(params[:cinema_id]).seats.find(params[:id])
    @time_slot = TimeSlot.where(seat_id: @seat.id )
  end
  ...
  private
    ...
    def seat_params
      params.require(:seat).permit(:name, :cinema_id)
    end
end
